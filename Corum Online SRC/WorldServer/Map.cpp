...
CMapWorldServer*	g_pMap[ MAX_WORLD_NUM_PER_SERVER ] = {0,};
DWORD	g_dwCurLandNum = 0; 


CMapWorldServer::CMapWorldServer()
{
	memset(this, 0, sizeof(CMapWorldServer));
...
//-----------------------------------------------------------------------------
void CMapWorldServer::Create(  DWORD dwID )
{
	if(ReadMap(dwID))
	{
...
// 속성 파일을 읽어드린다. 
//-----------------------------------------------------------------------------
BOOL CMapWorldServer::ReadMap(DWORD dwID)
{
...
// 지정해준 인덱스의 타일을 셋팅함. 
//-----------------------------------------------------------------------------
BOOL CMapWorldServer::SetMap(DWORD dwX, DWORD dwZ, MAP_TILE* pTile)
{
...
BOOL CMapWorldServer::SetTileOccupied( DWORD dwX, DWORD dwZ, BYTE attr, LPVOID pType )
{
	if(dwX >= m_dwTileNumWidth || dwZ >= m_dwTileNumHeight)
	{	
		return FALSE;
...
//-----------------------------------------------------------------------------
MAP_TILE* CMapWorldServer::GetMap(DWORD dwX, DWORD dwZ)
{
	if( dwX >= m_dwTileNumWidth || dwZ >= m_dwTileNumHeight )	
		return NULL;
...
// 좌표로 타일 포인터를 구함. 
//-----------------------------------------------------------------------------
MAP_TILE* CMapWorldServer::GetTile(float fx, float fz)
{
