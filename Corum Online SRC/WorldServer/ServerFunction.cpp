...
BOOL FindEmptyPosNearDungeon(BYTE bWorldID, VECTOR3* vpDungeonPos, VECTOR3 *vpNearPos, BOOL bVillage)
{
	...
	while(dwCount < 20)
	{
		...
		switch((dwCount % 4) + 1)
		{
			case 1:		vPos.x -= dwCount * (TILE_WIDTH * fDistance);	break;		//서쪽방향 
			case 2:		vPos.z -= dwCount * (TILE_WIDTH * fDistance);	break;		//남쪽방향
			case 3:		vPos.z += dwCount * (TILE_WIDTH * fDistance);	break;		//북쪽방향
			case 4:		vPos.x += dwCount * (TILE_WIDTH * fDistance);	break;		//동쪽방향 
		}		
